...
![电量图表](screenshots/电量图表.png)

> *图说：电量折线图主视图。灰阶折线 + 数据点、y 轴 0/25/50/75/100% 虚线参考、x 轴左端 `08-03 19:11` 右端 `08-10 19:11`、底部单一图例「电量曲线」（充放电均黑色实线 — KPW3 上灰线过淡，已统一为黑色）。*
...
![图表可选时间段](screenshots/图表可选时间段.png)

> *图说：「显示模式」菜单（顶栏左侧三横线按钮触发）。6 种时间窗（今天 / 近 7 天 / 近 30 天 / 近 90 天 / 近 180 天 / 近 365 天）+ 「打开休眠/唤醒追踪」跳转项；当前选中以 ASCII `>` 标记，不用 `✓`，避开 KPW3 cfont 缺字形。*

### 二、休眠·唤醒追踪

![休眠及唤醒电量变化追踪](screenshots/休眠及唤醒电量变化追踪.png)

> *图说：当日休眠/唤醒事件详细页（标题「休眠/唤醒事件」、首行事件日期 `2026-08-10` + 「查看电量图表」快捷）。6 维统计：总活跃时间、总休眠时间、活跃/休眠净电量变化、活跃/休眠平均耗电 %/小时、总计净变化（首尾校准）+ 续航估算。尾部唤醒·休眠周期倒序列表，每行 `唤醒→至今 (时长) [±电量%]` 或 `休眠→唤醒 (时长) [±电量%]`，单段低于 60 秒附 `[异常]`。*
